...
export async function POST(request: NextRequest) {
  try {
    ...
    let prompt = `Generate detailed writing instructions for each section of an investigation script.
...
- How this section connects to the overall investigation story
- Approximate word count for this section (aim for ~${Math.floor(wordCount / titles.length)} words per section)
...
Return ONLY the JSON array, no additional text or explanations.`;
    ...
    console.log(`📝 Generating writing instructions for ${titles.length} sections`);
    ...
    try {
      ...
      sections = titles.map((title: string) => ({
        title: title.trim(),
        writingInstructions: `Write a compelling section about "${title.trim()}" that contributes to the overall investigation narrative. Include relevant details, evidence, and maintain an engaging tone throughout. Target approximately ${Math.floor(wordCount / titles.length)} words for this section.`
      }));
    }

    console.log(`✅ Generated writing instructions for ${sections.length} sections`);

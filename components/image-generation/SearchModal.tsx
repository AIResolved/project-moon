...
import { Loader2, Search, Edit3, CheckSquare, Square, Image, Video, Download } from 'lucide-react'
import { ExtractedScene } from '@/types/image-generation'
...
interface SearchModalProps {
  ...
  mode: 'image' | 'video'
  onSelectResults: (results: SearchResult[]) => void
}

export function SearchModal({ isOpen, onClose, scenes, mode, onSelectResults }: SearchModalProps) {
      ...
      <DialogContent className="max-w-4xl h-[80vh] bg-gray-900 border-gray-700 text-white">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {mode === 'image' ? <Image className="h-5 w-5" /> : <Video className="h-5 w-5" />}
            Search Stock {mode === 'image' ? 'Images' : 'Videos'}
          </DialogTitle>
        </DialogHeader>
